...
#define ERR_TEXT(msg) std::format("##msg Error: {}",::GetLastError()).c_str()
...
inline Tv RoundDown(Tv Value, Tm Multiple) { return Value / Multiple * Multiple; }

template <typename Tv, typename Tm>
inline Tv RoundUp(Tv Value, Tm Multiple) { return RoundDown(Value, Multiple) + (Value % Multiple > 0 ? Multiple : 0); }

inline char* allocate_buffer_space(
    const DWORD buf_size,
    const DWORD buf_count,
    DWORD& total_buffer_size,
    DWORD& total_buffer_count)
{
    SYSTEM_INFO systemInfo;
    ::GetSystemInfo(&systemInfo);
    const unsigned __int64 granularity = systemInfo.dwAllocationGranularity;
    const unsigned __int64 desired_size = buf_size * buf_count;
    unsigned __int64 actual_size = RoundUp(desired_size, granularity);

    if (actual_size > std::numeric_limits<DWORD>::max())
    {
        actual_size = (std::numeric_limits<DWORD>::max() / granularity) * granularity;
    }
    total_buffer_count = std::min<DWORD>(buf_count, static_cast<DWORD>(actual_size / buf_size));
    total_buffer_size = static_cast<DWORD>(actual_size);
    const auto buffer_ptr = static_cast<char*>(VirtualAllocEx(GetCurrentProcess(), nullptr, total_buffer_size,
    ...
    )
{
    ...
    if (nullptr == g_h_iocp) 
        return std::format("CreateIoCompletionPort Error: {}",::GetLastError()).c_str();
    ...
    for (auto& [
        ...
        /// SEND용 RIO 버퍼 등록
        {
            DWORD total_buffer_count = 0;
            DWORD total_buffer_size = 0;
            
            send_buf_ptr = allocate_buffer_space(
                SEND_BUFFER_SIZE,
                RIO_PENDING_SENDS,
                total_buffer_size,
                total_buffer_count);
            
            send_buf_id = g_rio.RIORegisterBuffer(send_buf_ptr, total_buffer_size);

            if (send_buf_id == RIO_INVALID_BUFFERID) return ERR_TEXT(RIORegisterBuffer);
            
            DWORD offset = 0;

            send_buf = new EXTENDED_RIO_BUF[total_buffer_count];
            send_buf_count = total_buffer_count;

            for (DWORD j = 0; j < send_buf_count; ++j) {
        ...
        addr_buf_ptr = allocate_buffer_space(
            ADDR_BUFFER_SIZE,
            RIO_PENDING_RECVS,
            total_buffer_size,
            total_buffer_count);
        addr_buf_id = g_rio.RIORegisterBuffer(addr_buf_ptr, total_buffer_size);

        if (addr_buf_id == RIO_INVALID_BUFFERID) return ERR_TEXT(RIORegisterBuffer);

        DWORD offset = 0;
        addr_buf = new EXTENDED_RIO_BUF[total_buffer_count];
        addr_buf_count = total_buffer_count;

        for (DWORD j = 0; j < total_buffer_count; ++j) {
        ...
        recv_buf_ptr = allocate_buffer_space(
            RECV_BUFFER_SIZE,
            RIO_PENDING_RECVS,
            total_buffer_size,
            total_buffer_count);

        recv_buf_id = g_rio.RIORegisterBuffer(recv_buf_ptr, total_buffer_size);
        if (recv_buf_id == RIO_INVALID_BUFFERID) return ERR_TEXT(RIORegisterBuffer);
        
        offset = 0;

        const auto buffs = new EXTENDED_RIO_BUF[total_buffer_count];
            ...
            thread_number]
        {
            ...
            while (true) {
                ...
                for (DWORD i = 0; i < num_results; ++i) {
                    if (const auto buffer_ptr = reinterpret_cast<EXTENDED_RIO_BUF*>(results[i].RequestContext);
                        ...
                        EXTENDED_RIO_BUF* context = &send_buf[send_buf_index++ % send_buf_count];
                        ...
                        if (!g_rio.RIOSendEx(
                            ...
                            1,
                            nullptr,
                            &addr_buf[addr_buf_index % addr_buf_count],
                            nullptr,
                            nullptr,
                            0,
            ...
            return std::format(" -- thread[{} : id-{}] exit -- ", thread_number, std::this_thread::get_id()).c_str();
        });
